# Copyright (c) 2026, HV1.2 Sim-to-Sim
# SPDX-License-Identifier: BSD-3-Clause
"""
Deploy a trained HV1.2 velocity-tracking policy in MuJoCo.

HV1.2 v0 vs HV1 V3 — what this deploy script does NOT need:
  * No observation history (history_length=1, single-frame obs).
  * No end-effector pose commands.
  * No body_height / waist_alpha scalar commands.
  * No torso-IMU observations.
  * No base_lin_vel (real robot has no state estimator — trained without it).
  * Action dim is 12 (legs only). Non-leg joints stay at default via reset.

Observation layout (83-d, single frame, declaration order in PolicyCfg):
    [ 0:3 ]  base_ang_vel        (pelvis IMU, body frame)
    [ 3:6 ]  projected_gravity   (gravity in pelvis frame)
    [ 6:9 ]  velocity_commands   [vx, vy, wz]
    [ 9:40]  joint_pos           (q - q_default), 31 dims, Isaac order
    [40:71]  joint_vel           dq, 31 dims, Isaac order
    [71:83]  actions             previous action, 12 dims

Training added Unoise to base_ang_vel/projected_gravity/joint_pos/joint_vel for
robustness; at deploy we feed clean values (matches play.py and the real-robot
deployment direction).

YAML produced by:
  ./isaaclab.sh -p scripts/reinforcement_learning/rsl_rl/dump_mujoco_config.py \\
      --task Isaac-Velocity-Flat-HV1_2-Play-v0 --num_envs 1

Usage:
  python deploy/mujoco/deploy_mujoco_hv1_2.py \\
      --config <run_dir>/exported/mujoco_config.yaml \\
      --xml deploy/mujoco/hv1_2_scene.xml \\
      --cmd_lin_x 0.5
"""
...
def main():
    parser = argparse.ArgumentParser(description="Deploy HV1.2 velocity policy in MuJoCo.")
    ...
    parser.add_argument(
        "--xml", type=str,
        default="/home/rabisankar/IsaacLab/deploy/mujoco/hv1_2_scene.xml",
        help="MJCF scene file (includes hv1_2.xml).",
    )
